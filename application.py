...
def staff_view_flights():
	if 'staff' in session:
		...
		with cnx.cursor() as cur:
			if criteria == None and input_value == None:
				query = "SELECT * FROM flight WHERE airline_name = '{}' and flight.departure_time > '{}'".format(airline[0], one_month_ago)
			else:
...
def view_customers():
	if 'staff' in session:
		...
		with cnx.cursor() as cur:
			...
			airline = data[0]
		with cnx.cursor() as cur:
			query = "select purchases.customer_email, count(*)  from purchases natural join ticket where ticket.airline_name = '{}' and purchases.purchase_date > '{}' group by purchases.customer_email order by count(*) desc;".format(airline, one_year_ago)
			print(airline)
			cur.execute(query)
			data = cur.fetchall()
			print(data)
